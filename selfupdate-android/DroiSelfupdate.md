...
### Eclipse 安装SDK
1. 下载[SDK压缩包]()，解压后将 `droiselfupdatingsdk.jar`和`Utility.jar` 包导入到工程的 `libs` 目录下；右键工程根目录，选择`Properties` -> `Java Build Path` -> `Libraries`，然后点击`Add External JARs...` 选择指向jar的路径，点击OK，即导入成功。`（ADT17及以上不需要手动导入）`  
...
dependencies {
	/*其他依赖 */
	compile 'com.droi.sdk:selfupdating:+'
